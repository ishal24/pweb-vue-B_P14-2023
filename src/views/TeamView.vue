<template>
  <h1> Say Hello to Our Team from group P14! </h1>
  <h6> Meet the individuals who make our team unique and bring our vision to life. </h6>
  <br> 
  <div class="container">
    <div class="row">
      <div class="col-md-4">
        <div class="card">
          <img src="../components/img/cro.png" 
          class="card-img-top" 
          alt="...">
         <div class="card-body">
          <h5 class="card-title">Muhammad Faishal Rizki Ramadaniswara AhmadFauzan </h5>
          <a href="#" class="btn btn-primary">5027221026</a>
        </div>
       </div>
      </div>  

      <div class="col-md-4">
        <div class="card">
          <img src="../components/img/steph.jpg" 
          class="card-img-top" 
          alt="...">
         <div class="card-body">
          <h5 class="card-title">Stephanie Hebrina Mabunbun Simatupang</h5>
          <a href="#" class="btn btn-primary">5027221069</a>
        </div>
       </div>
      </div>  

      <div class="col-md-4">
        <div class="card">
        <img 
        src=""
        class="card-img-top" 
        alt="...">
         <div class="card-body">
          <h5 class="card-title">Made Gde Krisna Wangsa</h5>
          <a href="#" class="btn btn-primary">NRP</a>
        </div>
       </div>
      </div>  


    </div>
    
    
  </div>
</template>

<script>
export default {
  name: 'TeamView',
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>

</style>
