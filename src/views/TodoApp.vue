...
const newTodo = ref({
  text: '',
  category: 'High', // Default category
  completed: false, // Default is incomplete
});
...
function addTodo() {
  if (newTodo.value.text.trim() !== '') {
    ...
    newTodo.value.completed = false; // Set as incomplete by default
  }
}
...
button:hover {
  background-color: #0D6EFD; /* Darker red color on hover */
}
